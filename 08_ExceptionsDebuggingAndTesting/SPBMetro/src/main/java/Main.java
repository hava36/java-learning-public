...
public class Main {
    ...
    private static Logger searchLogger, inputErrorLogger, exceptionLogger;

    private static URI dataFile;

    static {
        try {
            dataFile = Main.class.getResource("map.json").toURI();
    ...
    private static void initLoggers() {
        searchLogger = LogManager.getLogger("search-logger");
        inputErrorLogger = LogManager.getLogger("input_error-logger");
        exceptionLogger = LogManager.getLogger("exception-logger");
    }
    ...
    private static Station takeStation(String message) {
        for (; ; ) {
            ...
            if (station != null) {
                searchLogger.info(String.format("Запрос станции %s", line));
                return station;
            }
            inputErrorLogger.info(String.format("Станция не найдена: %s", line));
